...
from prompts import PromptDescription, PromptModifier
import concurrent.futures


class ImageProcessorApp:
    def __init__(self):
        self.image_processor = ImageProcessor(input_size=64)
        self.llm = ChatOpenAI(**load_config(), timeout=300)

    def run(self, image_path: Path, captions: list):
        # 加载图像信息
        if isinstance(image_path, str):
            image_path = Path(image_path)
        assert image_path.exists(), f"Image file not found: {image_path}"

        image_base64 = self.image_processor.load_image(image_path)
        ...
        # 获取图像描述
        description = PromptDescription(self.llm).run(image_info, captions)
        desc_info = description.model_dump()

        # 根据描述修改图像信息
        modify = PromptModifier(self.llm).run(image_info, str(desc_info))
...
if __name__ == "__main__":
    ...
    with open("/home/yuyangxin/data/experiment/tmp.json", "r", encoding="utf-8") as file:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for path, captions in image_info.items():
            future_to_image[executor.submit(process_images, path, captions)] = path
            if len(future_to_image) >= 50:
                break
