...
def load_json(save_path: Path) -> dict:
    """加载JSON文件"""
    if save_path.is_file():
        print(f"[{save_path.stem}]: Json 文件已存在，加载文件")
...
class EntranceApp:
    def __init__(self):
        ...
        self.modify_mask_path: dict[ModifyType, Path] = {}
    ...
    def load_image_info(self, image_path: Path, mask_path: Path, detail_info: dict) -> tuple:
        """加载图像及其信息"""
        ...
        image_info = HumanMessage(
            content=[
                {"type": "text", "text": "The following image is a original image"},
                {"type": "image_url", "image_url": {"url": f"data:image/webp;base64,{self.image_processor.get_webp_base64(trans_img)}"}},
                {"type": "text", "text": "The following image is a mask image"},
                {"type": "image_url", "image_url": {"url": f"data:image/webp;base64,{self.image_processor.get_webp_base64(trans_mask)}"}},
            ]
        )
        ...
        return src_img, trans_img, scale_factor, src_mask, trans_mask, segmentation, image_info

    def run(self, image_path: Path, detail_info: dict) -> dict:
        image_path = Path(image_path)
        save_path = self.desc_save_path / f"{image_path.stem}.json"
        result = load_json(save_path)

        # 加载图像信息
        print(f"开始处理图片: {image_path}")
        mask_path = Path(detail_info["mask_path"])

        src_img, trans_img, scale_factor, src_mask, trans_mask, segmentation, image_info = self.load_image_info(
            image_path, mask_path, detail_info
        )
...
if __name__ == "__main__":
    ...
    # 保存错误信息
    if error_info:
        EntranceApp.save_json(Path("error_path.json"), error_info)
